...
class Maze:
    ...
    def display(self, mac__x:int, mac__y:int, state:str):
        #display labyrinth and elements                              
        self.window.fill((0, 70, 250))       
        self.window.blit(self.backgr_img, (20, 20))
    
        #run throught .structure[] to print each sprit matching letters in the txt file
    ...
    def randomize_items(self):    
        ...
        #run throught .structure[] searching for " " and create a random tuple positions 
        for y, col in enumerate(self.structure): 
            for x, case in enumerate(col):
                if case == " ":
